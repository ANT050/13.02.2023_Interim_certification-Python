...
Функцию adding_notes в модуле view.py - просит пользователя заполнить необходимые поля (заголовок и описание заметки) и считывает введенные пользователем данные. Возвращает кортеж значений (return header, description).
...
Вторая проверка проверка choice == 2. Вызывается функция adding_notes из модуля view.py (title, body = view.adding_notes()) для получения от пользователя введеных значений. Далее полученные значения от пользователя передаются в качестве аргумента функции adding_notes модуля model.py ( model.adding_notes(title, body)) для добавления заметки в файл notes.csv.
